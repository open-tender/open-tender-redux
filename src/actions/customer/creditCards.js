...
export const fetchCustomerCreditCards =
  includeLinked => async (dispatch, getState) => {
...
export const updateCustomerCreditCard =
  (cardId, data, callback) => async (dispatch, getState) => {
    const { api } = getState().config
    if (!api) return
    const token = selectToken(getState())
    if (!token)
...
export const removeCustomerCreditCard =
  (cardId, callback) => async (dispatch, getState) => {
    const { api } = getState().config
    if (!api) return
    const token = selectToken(getState())
    if (!token)
...
export const addCustomerCreditCard =
